...
def part4(keyword, regions, count, segmentation):
    ...
    # Store the main chapter number separately
    main_chapter = count
...
#     ch4 = "\n\n<strong>Chapter "+str(count)+": Global "+str(keyword)+" Market By Region</strong>\n\t"+str(count)+".1 Overview"
#     counter = 2
#     return ch4.replace("\n","<br />").replace("\t","&emsp;"),count
    
    # Start with the main chapter and overview section
    ch4 = f"\n\n<strong>Chapter {main_chapter}: Global {keyword} Market By Region</strong>\n\t{main_chapter}.1 Overview"
    
    # Split regions and segmentation
    t = regions.split(",")
    segment = segmentation.split("#")
    
    # Loop through each region
    for region_index, i in enumerate(t):
        region_chapter = f"{main_chapter}.{region_index + 2}"
        
        # Get the region name and countries
        region = i.split("(")
        
        # Add the region as a subchapter
        ch4 += f"\n\t<strong>{region_chapter}. {region[0].strip()} {keyword} Market</strong>"
        
        # Add subchapters for Key Market Trends, Top Key Companies, and Market Size by Segments
        ch4 += (f"\n\t\t{region_chapter}.1 Key Market Trends, Growth Factors and Opportunities"
                f"\n\t\t{region_chapter}.2 Top Key Companies"
                f"\n\t\t{region_chapter}.3 Historic and Forecasted Market Size by Segments")
        
        segment_counter = 4  # Start from 4 for the additional segments
        for s in segment:
            x = s.split("::")
            ch4 += f"\n\t\t{region_chapter}.{segment_counter} Historic and Forecasted Market Size By {x[0]}"
            y = x[1].split(",")
            for idx, z in enumerate(y):
                ch4 += f"\n\t\t{region_chapter}.{segment_counter}.{idx + 1} {z}"
            segment_counter += 1
        
        # Historic and Forecast Market Size by Country
        region[1] = region[1].replace(")", "")
        ch4 += f"\n\t\t{region_chapter}.{segment_counter} Historic and Forecast Market Size by Country"
        for idx, country in enumerate(region[1].split(",")[0].split(";")):    
            ch4 += f"\n\t\t{region_chapter}.{segment_counter}.{idx + 1} {country.strip()}"
    
    # Increment the main chapter count only by 1 for the next chapter
    new_count = main_chapter + 1
